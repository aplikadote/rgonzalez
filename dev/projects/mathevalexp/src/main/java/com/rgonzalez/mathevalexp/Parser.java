...
public class Parser {  
    ...
    */
    ...
    private boolean startsWith(Rule r, int symbolKind) {  
        ...
        boolean result;
        switch (ruleKind) {
            case Rule.THEN: {  Rule.Then rt = (Rule.Then) r;
                    result = startsWith(rules[rt.left], symbolKind);
                }
                break;
            case Rule.SKIP: {  Rule.Skip rk = (Rule.Skip) r;
                    result = rk.symbolKind == symbolKind;
                }
                break;
            case Rule.ACCEPT: {  Rule.Accept rs = (Rule.Accept) r;
